...
int main() {
    ...
    vector<int> cursed(N); // Adjusted to 0-based indexing
    for (int i = 0; i < N; ++i) {
        cin >> cursed[i];
    }

    Graph graph(N, true); // Directed graph

    for (int i = 0; i < M; ++i) {
        int u, v;
        cin >> u >> v;
        // Add edge only if both cities are not cursed
        if (!cursed[u] && !cursed[v]) {
            graph.add_edge(u, v);
            graph.add_edge(v, u); // Since roads are bidirectional
        }
    }

    // Check if source or target is cursed
    ...
    int result = bfs.min_dist(target);
